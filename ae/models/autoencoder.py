...
class AutoEncoder:
    ...
    def build_model(self):
        ...
        decoded = Dense(self.input_shape, activation="relu", activity_regularizer=regularizers.l2(0))(encoded3)
    ...
    def encode_data(self, data, csv_path, save_csv=True):
        ...
        train_coded = pd.DataFrame(coded_train, index=np.arange(len(coded_train)), columns=np.arange(self.encoding_dim))
        if save_csv:
