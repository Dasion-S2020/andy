...
class VGG(object):
    ...
    def _conv_block(self, block_num, inputs_x, filters, num_layers, pool_size=2, ignore_pooling=False):
        for i in range(num_layers):
            with tf.compat.v1.name_scope("conv_" + str(block_num) + "_" + str(i)):
                inputs_x = tf.compat.v1.layers.conv2d(
                    inputs=inputs_x,
                    filters=filters,
        ...
        if ignore_pooling:
            ...
            with tf.compat.v1.name_scope("max_pool_" + str(block_num)):
                max_pool_x = tf.compat.v1.layers.max_pooling2d(
                    inputs=inputs_x,
                    pool_size=pool_size,
                    strides=2,
                    padding="same"
    ...
    def build_vgg(self, inputs):
        ...
        if self.keep_prob is not None:
            flat = tf.compat.v1.layers.dropout(inputs=flat, rate=1.0-self.keep_prob, training=self.training)
