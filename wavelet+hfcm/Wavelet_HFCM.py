...
def analyze_paras_HFCM(dataset1, ratio, sp):
    ...
    for Oidx, Order in enumerate(Order_list):
        for Nidx, Nc in enumerate(Nc_list):
            ...
            if min_rmse < min_rmse_inall:
                ...
                best_alpha_scala = best_alpha
    if sp:
        np.save('best_predict.npy', best_predict_inall)
...
def analyze_parameter():
    ...
    dataset = sunspot[:-1, 1]
    ratio = 0.7674
    df1 = analyze_paras_HFCM(dataset, ratio=ratio, sp = False)
    ...
    df2 = analyze_paras_HFCM(dataset, ratio=0.6, sp = True)
